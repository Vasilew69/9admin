    ...
    if (data.markdown === true) {
        ...
        if (data.type === 'danger') {
            msgHtml += `<div class="text-right">
                <small>
                    For support, visit <strong><a href="http://discord.gg/nineadmin" target="_blank" class="text-dark">discord.gg/nineadmin</a></strong>.
                </small>
            </div>`;
        }
...
const nineadminAPI = ({type, url, data, dataType, timeout, success, error}) => {
    ...
    success = success || (() => {});
    error = error || (() => {});
    const headers = {'X-nineadmin-CsrfToken': (csrfToken) ? csrfToken : 'not_set'}
    // console.log(`nineadminAPI Req to: ${url}`);
    return $.ajax({type, url, timeout, data, dataType, success, error, headers});
};

const nineadminAlert = ({content, modalColor, title}) => {
    $.confirm({
        title,
        content: content,
...
const nineadminConfirm = ({content, confirmBtnClass, modalColor, title}) => {
...
(function () {
    ...
    const darkModeCookie = document.cookie.match(/(^| )nineadmin-darkMode=([^;]+)/);
    ...
    if (darkModeCookie === null) {
        ...
        if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
            ...
            document.cookie = 'nineadmin-darkMode=true;path=/';

        //If the user is on desktop
        } else if (window.location.pathname == '/') {
            const darkModeSuggestionCookie = document.cookie.match(/(^| )nineadmin-darkModeSuggestion=([^;]+)/);
            ...
            if (darkModeSuggestionCookie === null) {
                ...
                setTimeout(() => {
                    tooltip.show();
                    document.cookie = `nineadmin-darkModeSuggestion=true;expires=${expDate.toUTCString()};path=/`;
                }, 2000);
            }
        }
    }
    ...
    const handlerFn = function () {
        ...
        document.cookie = `nineadmin-darkMode=${isDarkMode};path=/`;
    };
